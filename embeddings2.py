...
if __name__ == "__main__":
    ...
    parser.add_argument('--full_graph', help='used for nearst neighbors regularization')
    ...
    wordpairs = helpers2.Wordpairs(trainfile)

    if not args.vocab_file:
        print('Building vocabulary from the training set...', file=sys.stderr)
        no_train_pairs, vocab_dict, inverted_vocabulary = helpers2.build_vocabulary(wordpairs)
    ...
    keras_model.compile(optimizer=adam, loss=helpers2.custom_loss(reg1_output, reg2_output, beta, gamma), metrics=['mse'])
    ...
    helpers2.save_word2vec_format(filename, vocab_dict, word_embedding_layer.get_weights()[0])
